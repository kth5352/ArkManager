...
- 재생 엔진을 네이티브 libmpv 기반으로 전면 교체 (안정성·성능 향상, 5밴드 오디오 이퀄라이저 프리셋 추가)
- 재생목록(플레이리스트) 생성·관리·좋아요, 현재 재생 큐 탭(드래그로 순서 변경) 추가
- `.lrc`에 더해 `.vtt`, `.ass` 자막 지원, 가사/자막 로그 탭 추가
- 자막만 따로 띄우는 별도 창(PIP) 기능 추가
- 폴더 탐색(브레드크럼, 뒤로/앞으로), ASMR 폴더 재생 지원
- 그리드/리스트 보기 전환과 확대 슬라이더 추가
- 전체화면 재생 UX 개선 (사이드바를 열어도 하단 재생바를 가리지 않도록 수정 등), 별도 창으로 분리(detach) 재생 지원

**UI 전면 개편**

- 디자인 토큰과 애니메이션 정책을 다시 세우고, 네비게이션/툴바, 툴팁, Gallery·List·DetailList, Explorer, Settings, 미디어 컨트롤, 세이브 관리 화면까지 시각·상호작용을 통일

**성능 개선**

- 폴더 스캔 시 동시 처리량 제한 및 응답 없는 항목 타임아웃 처리
- 썸네일 캐싱, 라이브러리 목록 데이터 안정화로 불필요한 재계산 감소

**이번 버전에서 새로 추가된 기능**

- 코드가 연동된(DLsite 등에서 자동으로 정보를 가져온) 게임도 표지 이미지를 직접 바꿀 수 있음
- 상세보기, List, DetailList에 제작사/서클 이름을 표시하고 클릭하면 해당 이름으로 바로 검색
- File 메뉴에 "전체 메타데이터 새로고침" 추가 - 이미 가져온 정보도 전부 다시 크롤링 (실행 전 확인 창 표시)
- 언어 설정을 바꾸면 프로그램 상단 메뉴(새로고침, 제외 항목 관리 등)에도 바로 반영

**버그 수정**

- 자막 로그에 ASS 자막의 장식용 도형 좌표 데이터가 그대로 노출되던 문제 수정
- 세이브 복원이 중간에 실패해도 기존 세이브가 안전하게 유지되도록 개선
- 그 밖에 다수의 안정성 문제 수정
