...
- 드라이브 전체(예: `F:`)를 라이브러리 경로로 등록했을 때, 컴퓨터에 따라 게임이 전혀 인식되지 않던 문제를 수정했습니다. 기존에 등록해두신 드라이브 경로도 자동으로 다시 보정됩니다.

### 1.2.2 (2026.09.12)

- 1.2.1에서 시도했던 수정이 근본 원인이 아니었습니다. VNDB(비주얼노벨) 게임 중 즐겨찾기·평점·세이브 등을 아직 하지 않은 항목이, 프로그램을 켤 때마다 정상적으로 수집해둔 정보가 삭제되고 다시 수집되던 문제를 수정했습니다. 이번에는 실제 원인을 찾아 고쳤습니다.

### 1.2.1 (2026.09.11)

- 프로그램을 켤 때마다 같은 메타데이터 항목 몇 개가 계속 다시 수집되던 문제를 수정했습니다. 정보를 가져오지 못한 항목은 이제 기록으로 남아 반복해서 재시도하지 않으며, 필요하면 파일 메뉴의 전체 메타데이터 새로고침으로 다시 시도할 수 있습니다.
